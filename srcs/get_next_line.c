...
/*   Updated: 2019/03/29 16:50:17 by apion            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static ssize_t	extract_line(char **line, t_gnl *gnl, int with_buff)
{
	size_t		size_in_buff;
	ssize_t		ret;

	size_in_buff = with_buff ? gnl->index_eol - gnl->index_start : 0;
	if (with_buff)
	...
	gnl->index_start = with_buff ? gnl->index_eol + 1 : 0;
	free(gnl->temp);
	gnl->temp_size = 0;
	gnl->temp = 0;
	if (!*line)
...
ssize_t			get_next_line(const int fd, char **line)
{
	...
	if (fd < 0 || !line)
		return (GNL_ERROR);
	while ((gnl.index_eol = gnl.index_start + memsearch(
					gnl.buff + gnl.index_start,
					gnl.buff_read - gnl.index_start + 1))
			>= gnl.index_start)
		return (extract_line(line, &gnl, EXTRACT_WITH_BUFFER));
